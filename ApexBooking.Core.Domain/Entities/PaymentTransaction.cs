...
public class PaymentTransaction : IAggregateRoot, ITenantEntity
{
    ...
    public void ReplaceGatewayOrder(string newGatewayTransactionId)
    {
        if (Status != PaymentTransactionStatus.Pending)
            throw new BusinessRuleBrokenException("Only pending transactions can be replaced.");

        if (string.IsNullOrWhiteSpace(newGatewayTransactionId))
            throw new BusinessRuleBrokenException("Gateway transaction ID is required.");
