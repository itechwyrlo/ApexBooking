...
public class Resource : IAggregateRoot, ITenantEntity
{
    ...
    private readonly List<ResourceAvailabilitySchedule> _availabilitySchedules = new();
    public IReadOnlyCollection<ResourceAvailabilitySchedule> AvailabilitySchedules => _availabilitySchedules.AsReadOnly();

    // Children: date-specific exceptions
    private readonly List<ResourceAvailabilityException> _availabilityExceptions = new();
    public IReadOnlyCollection<ResourceAvailabilityException> AvailabilityExceptions => _availabilityExceptions.AsReadOnly();
    ...
    /// </summary>
    public void SetWeeklySchedule(IEnumerable<ResourceAvailabilitySchedule> schedules)
    {
    ...
    public ResourceAvailabilitySchedule? GetScheduleForDay(DayOfWeek dayOfWeek)
    ...
    public void AddException(DateOnly exceptionDate, ExceptionType exceptionType, TimeOnly? startTime, TimeOnly? endTime, string? note)
    {
        ...
        var exception = ResourceAvailabilityException.Create(ResourceId, TenantId, exceptionDate, exceptionType, startTime, endTime, note);
        _availabilityExceptions.Add(exception);
        UpdatedAt = DateTime.UtcNow;
    }

    public void RemoveException(ResourceAvailabilityExceptionId exceptionId)
    {
        var exception = _availabilityExceptions.FirstOrDefault(e => e.ResourceAvailabilityExceptionId == exceptionId);
