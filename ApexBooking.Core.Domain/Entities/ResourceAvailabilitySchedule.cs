...
/// TR-7.4, UC-3.1.3
/// </summary>
public class ResourceAvailabilitySchedule : ITenantEntity
{
    ...
    private readonly List<ResourceBreakPeriod> _breakPeriods = new();
    public IReadOnlyCollection<ResourceBreakPeriod> BreakPeriods => _breakPeriods.AsReadOnly();

    protected ResourceAvailabilitySchedule() { }

    private ResourceAvailabilitySchedule(
        ...
        TimeOnly? endTime)
    {
        ResourceAvailabilityScheduleId = new ResourceAvailabilityScheduleId(Guid.NewGuid());
        ResourceId = resourceId;
        TenantId = tenantId;
        DayOfWeek = dayOfWeek;
    ...
    public static ResourceAvailabilitySchedule Create(
        ResourceId resourceId,
        TenantId tenantId,
        DayOfWeek dayOfWeek,
        bool isAvailable,
        ...
        TimeOnly? endTime)
    {
        ...
        return new ResourceAvailabilitySchedule(resourceId, tenantId, dayOfWeek, isAvailable, startTime, endTime);
    }
    ...
    public void AddBreakPeriod(TimeOnly breakStart, TimeOnly breakEnd, string? label = null)
    {
        ...
        var breakPeriod = ResourceBreakPeriod.Create(ResourceAvailabilityScheduleId, ResourceId, TenantId, breakStart, breakEnd, label);
        _breakPeriods.Add(breakPeriod);
        UpdatedAt = DateTime.UtcNow;
    }

    public void RemoveBreakPeriod(ResourceBreakPeriodId breakPeriodId)
    {
        var breakPeriod = _breakPeriods.FirstOrDefault(b => b.ResourceBreakPeriodId == breakPeriodId);
