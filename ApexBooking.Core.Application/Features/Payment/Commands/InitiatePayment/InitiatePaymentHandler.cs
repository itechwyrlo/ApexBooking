    ...
    : ICommandHandler<InitiatePaymentCommand, InitiatePaymentDto>
{
        ...
        CancellationToken ct)
    {
        ...
        var existingTransaction = await _unitOfWork.PaymentTransactionRepository
            .GetByBookingIdAsync(booking.BookingId, ct);

        if (existingTransaction is not null)
        {
            var existingApprovalUrl = await GetApprovalUrlAsync(
                accessToken, baseUrl, existingTransaction.GatewayTransactionId);

            if (existingApprovalUrl is not null)
            ...
            // Existing PayPal order expired — replace with a fresh one
            var replacementOrderId = await CreatePayPalOrderAsync(
                accessToken, baseUrl,
                booking.PriceSnapshot, booking.CurrencyCode,
                booking.BookingReference, returnUrl, cancelUrl);

            if (replacementOrderId is null)
                throw new InvalidOperationException("Failed to create PayPal order.");

            var replacementApprovalUrl = await GetApprovalUrlAsync(accessToken, baseUrl, replacementOrderId);

            if (replacementApprovalUrl is null)
                throw new InvalidOperationException("Failed to retrieve PayPal approval URL.");

            existingTransaction.ReplaceGatewayOrder(replacementOrderId);
            await _unitOfWork.CompleteAsync(ct);

            return new InitiatePaymentDto(
                ApprovalUrl: replacementApprovalUrl,
                GatewayTransactionId: replacementOrderId,
                BookingReference: booking.BookingReference);
        }
