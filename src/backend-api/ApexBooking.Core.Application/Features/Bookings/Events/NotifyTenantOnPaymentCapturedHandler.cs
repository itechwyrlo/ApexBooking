...
{
    public class NotifyTenantOnPaymentCapturedHandler
        : INotificationHandler<DomainEventNotification<PaymentCapturedDomainEvent>>
    {
        private readonly IUnitOfWork _unitOfWork;
        private readonly IRealtimeNotificationDispatcher _realtimeDispatcher;
        private readonly ILogger<NotifyTenantOnPaymentCapturedHandler> _logger;

        public NotifyTenantOnPaymentCapturedHandler(
            IUnitOfWork unitOfWork,
            IRealtimeNotificationDispatcher realtimeDispatcher,
            ILogger<NotifyTenantOnPaymentCapturedHandler> logger)
        {
        ...
        public async Task Handle(DomainEventNotification<PaymentCapturedDomainEvent> notification, CancellationToken cancellationToken)
        {
            ...
            {
                _logger.LogWarning(
                    "Could not resolve an Owner for Tenant {TenantId}; payment-captured notification skipped for {BookingReference}.",
            ...
            var n = Notification.Create(
                ...
                "Payment Received",
                $"Payment of {e.AmountDue:0.00} {e.CurrencyCode} was received for booking {e.BookingReference}.");
