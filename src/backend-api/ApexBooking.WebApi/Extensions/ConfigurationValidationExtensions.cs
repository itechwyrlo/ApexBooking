...
public static class ConfigurationValidationExtensions
{
        ...
        IWebHostEnvironment environment)
    {
        ...
        if (string.IsNullOrEmpty(JwtSettings?.PrivateKeyPem) ||
            string.IsNullOrEmpty(JwtSettings?.PublicKeyPem))
        {
            throw new InvalidOperationException(
        ...
        // Validate the QR boarding-pass signing key (Phase 5 — secure ticket tokens)
        var securityOptions = configuration.GetSection("Security").Get<SecurityOptions>();
        if (string.IsNullOrWhiteSpace(securityOptions?.TicketSigningKey) || securityOptions.TicketSigningKey.Length < 32)
        {
            throw new InvalidOperationException(
                "Security:TicketSigningKey must be configured with at least 32 characters. " +
                "Set via environment variable Security__TicketSigningKey or User Secrets");
        }

        // Deliberately separate key from TicketSigningKey (see HmacCancellationTokenService) —
        // must be validated here too, or a missing key only surfaces as a silent, permanently-failed
        // outbox message the next time a booking is confirmed, instead of a boot-time failure.
        if (string.IsNullOrWhiteSpace(securityOptions?.CancellationTokenSigningKey) || securityOptions.CancellationTokenSigningKey.Length < 32)
        {
            throw new InvalidOperationException(
                "Security:CancellationTokenSigningKey must be configured with at least 32 characters. " +
                "Set via environment variable Security__CancellationTokenSigningKey or User Secrets");
        }
