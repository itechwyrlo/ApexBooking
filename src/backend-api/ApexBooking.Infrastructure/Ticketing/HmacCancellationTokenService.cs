...
{
    // Mirrors HmacTicketTokenService's shape exactly, but signed with its own key and packing
    // only BookingId + TenantId — kept structurally independent from the admission ticket token
    // so the two are never interchangeable (see ICancellationTokenService's doc comment).
    public sealed class HmacCancellationTokenService : ICancellationTokenService
    {
        private const int SignatureBytes = 16; // truncated HMAC — 128-bit forgery resistance
        private const int PayloadBytes = 32;   // BookingId + TenantId, 16 bytes each

        private readonly byte[] _key;

        public HmacCancellationTokenService(IOptions<SecurityOptions> options)
        {
            var secret = options.Value.CancellationTokenSigningKey;
            if (string.IsNullOrWhiteSpace(secret) || secret.Length < 32)
                throw new InvalidOperationException("Security:CancellationTokenSigningKey must be at least 32 characters.");

            _key = Encoding.UTF8.GetBytes(secret);
        }

        public string Issue(CancellationTokenPayload payload)
        {
        ...
        public bool TryValidate(string token, out CancellationTokenPayload payload)
        {
            payload = default;
        ...
        private static byte[] Pack(CancellationTokenPayload payload)
        {
        ...
        private static CancellationTokenPayload Unpack(ReadOnlySpan<byte> body) => new(
