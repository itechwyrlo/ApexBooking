...
const ResourceAvailabilityPage: React.FC = () => {
  const { tenant, id: resourceId } = useParams<{ tenant: string; id: string }>();
  ...
  const {
    ...
    addException,
    removeException,
  } = useResourceAvailability(resourceId!);
    ...
    <div className="container-fluid">
      <div className="d-flex align-items-center gap-3 mb-4">
        <button
          ...
          onClick={() => navigate(`/t/${tenant}/resources`)}
        >
          <FontAwesomeIcon icon={faArrowLeft} className="me-1" />
          Back
        </button>
        <div>
          <h4 className="fw-bold mb-0">Resource Availability</h4>
          <div className="text-muted small">Set weekly schedule and date-specific exceptions.</div>
        </div>
      </div>
      ...
      <div className="card border-0 shadow-sm mb-4">
        <div className="card-header bg-white border-bottom py-3">
          ...
          <div className="text-muted small mt-1">
            Defines the recurring working hours for this resource. This is a full replacement on save.
          </div>
        </div>
                ...
                <div className="mt-2 ps-4 d-flex flex-column gap-2">
                  {day.breaks.map((brk, breakIndex) => (
                    <div key={breakIndex} className="d-flex align-items-center gap-2">
                      <span className="text-muted small" style={{ minWidth: 40 }}>Break</span>
                      <input
                        type="time"
                        className="form-control form-control-sm"
                        style={{ width: 120 }}
