...
import { useFirebaseMessaging } from '../../features/notifications/hooks/useFirebaseMessaging';
...
export const Header: React.FC<HeaderProps> = ({ onToggleSidebar }) => {
  ...
  useSignalR({ appendNotification });
  const { requestPermissionAndSetup, currentFcmToken } = useFirebaseMessaging({ appendNotification });

  const { logout, isLoading: loggingOut } = useLogout(currentFcmToken);
        ...
        <div className="position-relative" ref={notifRef}>
          <button
            ...
            onClick={() => {
              setNotifOpen(p => !p);
              setUserMenuOpen(false);
              // replaced by real-time SignalR + FCM — fetch now on mount only
            }}
